...
const LowActivationReport = () => {
  const [sortOrder, setSortOrder] = useState("");
  const [entriesPerPage, setEntriesPerPage] = useState(10);
  const [lowactivation, setLowActivation] = useState([]); // table data
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        const res = await axios.get("http://localhost:5000/api/lowactivationreport");
        setLowActivation(res.data); // assume response is an array of objects
      } catch (err) {
   ...
   // 🔹 Sort logic
  const sortedData = [...lowactivation].sort((a, b) => {
  ...
  const downloadExcel = () => {
    ...
    XLSX.utils.book_append_sheet(workbook, worksheet, "Low Activation Report");
    ...
    saveAs(fileData, "lowactivation_report.xlsx");
  };
  ...
  return (
    <div className="p-2 rounded-md shadow-2xl">
      <div className="w-full py-2 flex justify-between bg-gray-300">
        <h2 className="px-2 font-semibold">Low Activation Report</h2>
          ...
          <table className="min-w-full border-collapse text-sm">
            <thead className="bg-gray-50">
              <tr className="text-gray-600 border-b">
                ...
                <th className="px-4 py-2 text-[12px] text-left font-medium">CompanyName</th>
                ...
                <th className="px-4 py-2 text-[12px] text-left font-medium">Activated Keys</th>
            ...
            <tbody>
              {sortedData.slice(0, entriesPerPage).map((item, index) => (
                <tr key={item.id || index} className="hover:bg-gray-50 border-b">
                  ...
                  <td className="px-4 py-2">{item.activatedKeys}</td>
...
export default LowActivationReport;
