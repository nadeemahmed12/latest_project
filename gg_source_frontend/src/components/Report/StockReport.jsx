...
const StockReport = () => {
  ...
  const [stockData, setStockData] = useState([]); // table data
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        const res = await axios.get("http://localhost:5000/api/stockreport");
        setStockData(res.data); // assume response is an array of objects
      } catch (err) {
  ...
  const downloadExcel = () => {
    ...
    saveAs(fileData, "stock_report.xlsx");
  };
  ...
  return (
    <div className="p-2 rounded-md shadow-2xl">
      <div className="w-full py-2 flex justify-between bg-gray-300">
        <h2 className="px-2 font-semibold">Stock Report</h2>
          ...
          <table className="min-w-full border-collapse text-sm">
            <thead className="bg-gray-50">
              <tr className="text-gray-600 border-b">
                ...
                <th className="px-4 py-2 text-[12px] text-left font-medium">Name</th>
                <th className="px-4 py-2 text-[12px] text-left font-medium">Company</th>
                ...
                <th className="px-4 py-2 text-[12px] text-left font-medium">Smart Keys</th>
                <th className="px-4 py-2 text-[12px] text-left font-medium">Free Keys</th>
                <th className="px-4 py-2 text-[12px] text-left font-medium">Ultra Keys</th>
                <th className="px-4 py-2 text-[12px] text-left font-medium">Ultra Free Keys</th>
                <th className="px-4 py-2 text-[12px] text-left font-medium">Last Purchase</th>
              </tr>
            </thead>
            ...
            <tbody>
              {sortedData.slice(0, entriesPerPage).map((item, index) => (
                <tr key={item.id || index} className="hover:bg-gray-50 border-b">
                  ...
                  <td className="px-4 py-2">{item.lastPurchase}</td>
                </tr>
              ))}
            </tbody>
          </table>
